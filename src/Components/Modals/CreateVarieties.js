...
function CreateVarieties({ isOpen, onClose, varieties }) {
  const [varietyName, setCategoryName] = useState('');

  const handleAddVariety = () => {
    if (varietyName) {
      alert(`Variety added: ${varietyName}`);
      setCategoryName('');
    } else {
    ...
    >
      ...
      <div
        className={`relative bg-gray-900 text-white border border-border w-[80%] md:w-[500px] lg:w-[600px] p-8 rounded-2xl transform transition-transform duration-500 ease-in-out z-50 ${
          isOpen ? '-translate-y-6' : '-translate-y-10'
        }`}
      >
        {/* Header */}
        <div className="flex justify-center items-center mb-6">
          <h2 className="text-xl font-bold text-center w-full">Create Varieties</h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-6 p-4">
          {/* Category Name Input */}
          <div>
            <label className="block text-sm font-semibold mb-2 p-3 text-border">Variety Name</label>
            <input
              type="text"
              value={varietyName}
...
export default CreateVarieties;